from crossover_2019 import uniform_cross, pmx_cross, order_cross
from utils import run_multiple, muta_bin, tour_sel, sel_survivors_elite
from jb import fitness
...
if __name__ == '__main__':
    ...
    if True:
        for prob_mut in probs_mut:
            for prob_cross in probs_cross:
                filename = f"results\parameterization\{prob_mut}mut-{prob_cross}crossGGG.csv"
                print(f"Running test for {prob_mut} mutation prob and {prob_cross} crossover prob")
                run_multiple(filename,num_runs,numb_generations,size_pop, size_cromo, prob_mut,prob_cross,sel_parents,recombination,mutation,sel_survivors, fitness_func)
